...
const AdminOrders = () => {
  ...
  const handleSort = (sortOption) => {
    ...
    console.log({ sort });
    setSort((prevSort) => ({ ...prevSort, ...sort }));
  }
              ...
              <table className="min-w-max w-full table-auto">
                <thead>
                  <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                    <th className="py-3 px-6 text-left cursor-pointer" onClick={() => handleSort(
                      { sort: sort._sort === 'id' ? (sort?._sort === 'price' ? '-price' : 'price') : 'id' }
                    )}>Order# {' '}
                      {sort._sort === 'id' && 
                      (sort._sort === 'price' ? (<ArrowUpIcon className='w-4 h-4 inline' />
                    ) : (  
                      <ArrowDownIcon className='w-4 h-4 inline'/>))} 
                    </th>
                    <th className="py-3 px-6 text-left">Items</th>
                    <th className="py-3 px-6 text-left cursor-pointer" onClick={() => handleSort(
                      { sort: sort._sort === 'totalAmount' ? (sort?._sort === '-price' ? 'price' : '-price') : 'totalAmount' }
                    )}>Total Amount {' '}
                      {sort._sort === 'totalAmount' && 
                      (sort._sort === '-price' ? (<ArrowDownIcon className='w-4 h-4 inline' />
                    ) : (  
                ...
                <tbody className="text-gray-600 text-sm font-light">
                  {orders.map((order) => <tr key={order.id} className="border-b border-gray-200 hover:bg-cyan-100">
                      ...
                      {order.items.map((item, index) => <div key={index} className="flex items-center">
                        <div className="mr-2">
                          <img
                            ...
                            src={item.thumbnail} alt='imagee'
                          />
                        </div>
                        <span>{item.name} - #{item.quantity} - ${discountedPrice(item)}</span>
                      </div>)}
                    </td>
                    ...
                    <td className="py-3 px-6 text-center">
                      {order.id === editableOrderId ? (
                        <select onChange={(e) => handleUpdate(e, order)} className="bg-purple-200 text-purple-600 py-2 px-6 rounded-full text-xs">
